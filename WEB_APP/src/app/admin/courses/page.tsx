...
export default function CoursesManagementPage() {
  ...
  const handleSyncMoodle = async () => {
    if (!confirm('确定要从 Moodle 同步所有课程吗？\n\n这将创建新课程或更新现有课程信息。')) {
      return;
    }
    ...
    try {
      ...
      toast({
        title: '✅ 同步成功',
        description: `成功同步 ${data.data.synced} 门课程（新增 ${data.data.created}，更新 ${data.data.updated}）`,
      });
    ...
    <div className="container mx-auto py-8 px-4">
      <div className="mb-8 flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-600 mt-2">管理系统中的所有课程</p>
        </div>
        <Button
        ...
        <div className="flex items-start justify-between">
          <div className="flex-1">
            ...
            <p className="text-sm text-gray-600 mb-4">
              从 Moodle 平台同步所有课程到系统中。同步后，学生添加课程时可以选择这些课程。
            </p>

            {stats && (
              <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 mb-4">
                <h3 className="font-medium text-blue-900 mb-2">上次同步结果：</h3>
                <div className="grid grid-cols-2 md:grid-cols-5 gap-4 text-sm">
                  <div>
                    <div className="text-blue-600 font-semibold">
                      {stats.totalMoodleCourses}
                    </div>
                    <div className="text-blue-800">Moodle 课程</div>
                  </div>
                  <div>
                    <div className="text-green-600 font-semibold">
                      {stats.synced}
                    </div>
                    <div className="text-green-800">成功同步</div>
                  </div>
                  <div>
                    <div className="text-emerald-600 font-semibold">
                      {stats.created}
                    </div>
                    <div className="text-emerald-800">新创建</div>
                  </div>
                  <div>
                    <div className="text-amber-600 font-semibold">
                      {stats.updated}
                    </div>
                    <div className="text-amber-800">已更新</div>
                  </div>
                  <div>
                    <div className="text-gray-600 font-semibold">
                      {stats.skipped}
                    </div>
                    <div className="text-gray-800">已跳过</div>
                  </div>
                </div>
                {stats.moodleSite && (
                  <p className="text-xs text-blue-700 mt-3">
                    Moodle 站点: {stats.moodleSite} ({stats.moodleVersion})
                  </p>
                )}
              </div>
            )}
                    ...
                    <tr key={course.courseId} className="hover:bg-gray-50">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        <span className={`px-2 py-1 inline-flex text-xs leading-5 font-semibold rounded-full ${
                          ...
                          {course.status === 'active' ? '活跃' : course.status === 'archived' ? '已归档' : '已取消'}
                        </span>
                      </td>
