...
export function EditGradesDialog({ enrollment, isOpen, onClose, onSuccess }: EditGradesDialogProps) {
  ...
  const t = useTranslations('dialogs.editGrades');
  const tCommon = useTranslations('dialogs.common');
  const tChange = useTranslations('dialogs.changeStatus');
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="space-y-4 border rounded-lg p-4 bg-blue-50">
            ...
            <div className="grid grid-cols-1 gap-4">
              <div>
                <Label htmlFor="midtermMark">{t('midterm')} (0-100)</Label>
                <Input
                  id="midtermMark"
                  type="number"
                  step="0.01"
              ...
              <div>
                <Label htmlFor="midtermComments">{t('comments')}</Label>
                <Textarea
                  id="midtermComments"
          ...
          <div className="space-y-4 border rounded-lg p-4 bg-green-50">
            ...
            <div className="grid grid-cols-1 gap-4">
              <div>
                <Label htmlFor="finalGrade">{t('final')} (0-100)</Label>
                <Input
                  id="finalGrade"
                  type="number"
                  step="0.01"
              ...
              <div>
                <Label htmlFor="finalComments">{t('comments')}</Label>
                <Textarea
                  id="finalComments"
          ...
          <div className="bg-gray-50 rounded-lg p-4 text-sm">
            <h4 className="font-semibold text-gray-700 mb-2">{tChange('courseInfo')}</h4>
            <div className="grid grid-cols-2 gap-2 text-gray-600">
              <div>{tChange('student')}: {enrollment.studentName}</div>
              <div>{tChange('course')}: {enrollment.courseName}</div>
              <div>{tChange('teacher')}: {enrollment.teacherName || '-'}</div>
              <div>{tChange('semester')}: {enrollment.academicYear} - {enrollment.semester}</div>
            </div>
          </div>
